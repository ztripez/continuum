...
export function GridHeatmap({ samples, label }: GridHeatmapProps) {
  ...
  useEffect(() => {
    ...
    const xVals = [...new Set(scalarSamples.map((s) => s.position[0]))].sort((a, b) => a - b);
    const yVals = [...new Set(scalarSamples.map((s) => s.position[1]))].sort((a, b) => a - b);

    const colorExtent = d3.extent(scalarSamples, (d) => d.scalar!) as [number, number];
    const color = d3.scaleSequential(d3.interpolateInferno).domain(colorExtent);

    const x = d3.scaleBand<number>().domain(xVals).range([0, w]).padding(0.02);
    const y = d3.scaleBand<number>().domain(yVals).range([h, 0]).padding(0.02);
    ...
    const xTickValues = xVals.filter((_, i) => i % Math.ceil(xVals.length / xTickCount) === 0);
    const yTickValues = yVals.filter((_, i) => i % Math.ceil(yVals.length / yTickCount) === 0);
