...
struct Cli {
    ...
    /// Directory containing frontend assets (defaults to static/ next to binary)
    #[arg(long)]
...
async fn main() {
    ...
    info!("Continuum Inspector listening on http://{}", cli.bind);
...
async fn proxy_socket(mut websocket: WebSocket, state: AppState) {
    ...
    // Task: read from Unix socket, send to WebSocket
    ...
    // Task: read from WebSocket, send to Unix socket
