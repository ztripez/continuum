...
use crate::ast::{
    BlockBody, ExecutionBlock, ObserveBlock, ObserveWhen, Stmt, WarmupBlock, WhenBlock,
};
use crate::lexer::Token;

/// Parse execution blocks inside a primitive declaration body.
pub fn parse_execution_blocks(stream: &mut TokenStream) -> Result<Vec<ExecutionBlock>, ParseError> {
    let mut blocks = Vec::new();

    while !matches!(stream.peek(), Some(Token::RBrace)) {
        match stream.peek() {
            Some(Token::Resolve) => {
                blocks.push(parse_execution_block(stream, Token::Resolve)?);
            }
            Some(Token::Collect) => {
                blocks.push(parse_execution_block(stream, Token::Collect)?);
            }
            Some(Token::WarmUp) => {
                blocks.push(ExecutionBlock::WarmUp(parse_warmup_block(stream)?));
            }
            Some(Token::When) => {
                blocks.push(ExecutionBlock::When(parse_when_block(stream)?));
            }
            Some(Token::Observe) => {
                blocks.push(ExecutionBlock::Observe(parse_observe_block(stream)?));
            }
            other => {
                return Err(ParseError::unexpected_token(
                    other,
                    "execution block keyword",
                    stream.current_span(),
                ));
            }
        }
...
/// Parse a standard execution block (resolve/collect).
fn parse_execution_block(
    ...
    match keyword {
        Token::Resolve => Ok(ExecutionBlock::Resolve { body, span }),
        Token::Collect => Ok(ExecutionBlock::Collect { body, span }),
        _ => unreachable!(),
    }
}

/// Parse warmup block.
fn parse_warmup_block(stream: &mut TokenStream) -> Result<WarmupBlock, ParseError> {
    let start = stream.current_pos();
...
/// Parse when block.
fn parse_when_block(stream: &mut TokenStream) -> Result<WhenBlock, ParseError> {
    let start = stream.current_pos();
...
/// Parse observe block.
fn parse_observe_block(stream: &mut TokenStream) -> Result<ObserveBlock, ParseError> {
    let start = stream.current_pos();
    stream.expect(Token::Observe)?;

    let when = match stream.peek() {
        Some(Token::When) => {
            stream.advance();
            stream.expect(Token::LBrace)?;
            let condition = super::expr::parse_expr(stream)?;
            stream.expect(Token::RBrace)?;
            Some(ObserveWhen { condition })
        }
        _ => None,
    };
